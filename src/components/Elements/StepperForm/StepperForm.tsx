...
import { FormBuilder } from '@shubham-chavda/react-custom-components';
        ...
        <>
            <div className="stepper-container-preview">
                ...
                <div className='json_wrapper'>
                    ...
                    <FormBuilder jsonData={props?.stepperJson?.[selectedStepperNumber]} />
                </div>
                <div className='stepper_btn_container_preview'>
                    <button onClick={() => setSelectedStepperNumber((prev) => prev - 1)} disabled={selectedStepperNumber < 2}>Previous</button>
                    <button onClick={() => {
                        selectedStepperNumber < +props?.stepperJson?.numSteps ? setSelectedStepperNumber((prev) => prev + 1) : null
                    }
                    } >{selectedStepperNumber === +props?.stepperJson?.numSteps ? "Finish" : "Next"}</button>
                </div>
            </div>
        </>
    )
