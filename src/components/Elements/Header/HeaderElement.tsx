...
const HeaderElement = ({ headerProps }) => {
    console.log(headerProps.properties.headerDetails.imageDetails.dataURL)
    ...
    return (
        ...
        >
            <div onClick={() => replaceRoute("/")} className="flex items-center pointer" style={headerProps?.properties?.style}>
                <img
                    src={
                        headerProps?.properties?.headerDetails?.imageDetails?.dataURL || bvLogo
                    }
                    className="icon-30"
                    alt="header-logo"
                />
            ...
            <div className="flex items-center">
                <ul className="flex items-center camelCase p0 m0 ul-styles">
                    {headerProps?.properties?.menuDetails.map((item, i) => {
                        return (
                            <li onClick={() => replaceRoute(item?.link)} className="p2 header_link pointer" key={i}>
